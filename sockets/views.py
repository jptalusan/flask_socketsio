import os
from flask import render_template, request, send_from_directory, url_for, redirect, flash, Blueprint
...
def upload():
    if request.method == 'POST':
        ...
        if f and allowed_file(f.filename):
            ...
            f.save(os.path.join(route_blueprint.config['UPLOAD_FOLDER'], filename))
            return redirect(url_for('.uploaded_file', filename = filename))
        return redirect(url_for('.index'))

@route_blueprint.route('/update_configs', methods=['POST'])
def update_configs():
...
@route_blueprint.route('/uploads/<path:filename>', methods=['GET', 'POST'])
